...
int main(int argc, char **argv) {
    ...
    while (fgets(cmd, CMD_LEN, stdin) != NULL) {
        ...
        // Task 6: Wait for all background jobs
            ...
            // Check if the last token is "&", indicating a background job
            if (tokens.length > 0 && strcmp(strvec_get(&tokens, tokens.length - 1), "&") == 0) {
                background = 1;
                strvec_take(&tokens, tokens.length - 1);    // Remove "&" from token list
            }
            ...
            if (child_pid == -1) {
                ...
                if (!background) {    // Foreground execution
                    tcsetpgrp(STDIN_FILENO, child_pid);    // Give terminal control to child

                    int status;
                    waitpid(child_pid, &status, WUNTRACED);

                    tcsetpgrp(STDIN_FILENO, getpid());    // Restore terminal to shell

                    if (WIFSTOPPED(status)) {    // Handle stopped jobs
